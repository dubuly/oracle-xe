...
-- SUBSTR() 함수: 지정된 길이의 문자열을 추출
SELECT SUBSTR('HelloWorld', 1, 5) FROM dual;

-- LENGTH() 함수: 문자열 길이를 숫자 값으로 표시
SELECT LENGTH('HelloWorld') FROM dual;

-- INSTR() 함수: 이름이 지정된 문자의 숫자 위치를 찾음
SELECT INSTR('HelloWorld', 'W') FROM dual;

-- LPAD() 함수: 길이가 n이 되도록 왼쪽부터 문자식으로 채운 표현식을 반환
SELECT LPAD('Salary', 10, '*') FROM dual;

-- RPAD() 함수: 길이가 n이 되도록 오른쪽부터 문자식으로 채운 표현식을 반환
SELECT RPAD('Salary', 10, '*') FROM dual;

-- REPLACE() 함수: 문자열 치환
SELECT REPLACE('JACK and JUE', 'J', 'BL') FROM dual;

-- TRIM() 함수: 문자열에서 선행 문자나 후행 문자를 자름
SELECT TRIM('H' FROM 'HelloWorld') FROM dual;

/*
숫자 함수
*/
-- ROUND() 함수: 지정된 소수점 자릿수로 값을 반올림
SELECT ROUND(45.926,2) FROM dual;

-- TRUNC() 함수: 지정된 소수점 자릿수로 값을 truncate 합니다.
...
/*
NVL2() 함수
    첫번째 표현식을 검사합니다. 첫번째 표현식이 null이 아니면 NVL2 함수는 두번째
    표현식을 반환합니다.
...
/*
NULLIF() 함수
    두 표현식을 비교하여 같으면 null 반환, 다르면 expr1을 반환합니다.
