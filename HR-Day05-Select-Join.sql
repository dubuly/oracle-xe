...
/*
USING 절로 조인 생성
    Natural join 은 이름과 데이터 유형이 대응되는 모든 열을 사용하여
    테이블 조인 한다.
*/
...
/*
Nonequijoin
    Nonequijoin은 등호 연산자 외의 다른 연산자를 포함하는 조인 조건이다.
...
LEFT OUTER JOIN
    ...
    왼쪽 테이블인 EMPLOYEES 테이블의 모든 행을 검색한다.
*/
...
/*
RIGHT OUTER JOIN
    EMPLOTEES 테이블에 대응되는 행이 없어도
    오른쪽 테이블인 DEPARTMENTS 테이블의 모든 행을 검색한다.
*/
SELECT e.last_name, e.department_id, d.department_name
...
/*
Cartesian Product
    조인 조건이 잘못되거나 완전히 생략된 경우 결과는 모든 행 조합이
    표시괴는 Cartesian Product로 나타난다.
*/

/*
Cross Join 생성
    CROSS JOIN 절은 두 테이블의 교차 곱을 생성한다.
*/
